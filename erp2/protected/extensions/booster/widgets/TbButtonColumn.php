...
class TbButtonColumn extends CButtonColumn {
	...
	protected function renderButton($id, $button, $row, $data) {
		...
		//下面三行是自己加的代码，为了实现每行onclick事件不一样
		if (!isset($options['onclick'])&&isset($button['onclick'])) {
			$options['onclick'] =  $this->evaluateExpression($button['onclick'], array('data' => $data, 'row' => $row));
		}
