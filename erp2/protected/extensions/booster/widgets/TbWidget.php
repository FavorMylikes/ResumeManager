...
abstract class TbWidget extends CWidget {
	...
	protected function isValidContext($context = false) {
		if($context)
			return defined(get_class($this).'::CTX_'.strtoupper($context));
		else
			return defined(get_class($this).'::CTX_'.strtoupper($this->context));
	}
	...
	protected function getContextClass($context = false) {
		if($context)
			return constant(get_class($this).'::CTX_'.strtoupper($context).'_CLASS');
		else
			return constant(get_class($this).'::CTX_'.strtoupper($this->context).'_CLASS');
	}
}
